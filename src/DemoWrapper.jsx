...
import App, { AdminPanel } from "./App.jsx";
...
export default function DemoWrapper() {
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "#f5f5f5", fontFamily: "'Barlow', system-ui, sans-serif", color: "#1a1a1a" }}>
      ...
      <aside style={{ width: 320, flexShrink: 0, background: "#fff", borderRight: "1px solid #e5e7eb", display: "flex", flexDirection: "column", position: "fixed", top: 0, left: 0, bottom: 0, zIndex: 10, overflowY: "auto", scrollbarWidth: "none", msOverflowStyle: "none" }}>
        ...
        <div style={{ padding: "20px 24px 16px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{ width: 44, height: 44, borderRadius: 12, background: accent, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Vollkorn', serif", fontSize: 22, fontWeight: 700, color: "#fff" }}>
              {c.studio.logo}
            </div>
            <div>
              <div style={{ fontFamily: "'Vollkorn', serif", fontSize: 18, fontWeight: 600, letterSpacing: "0.02em", color: "#1a1a1a" }}>{c.studio.name}</div>
              <div style={{ fontSize: 11, color: "#9ca3af", marginTop: 1 }}>{c.studio.tagline}</div>
            </div>
          </div>
        </div>
      ...
      <main style={{ flex: 1, display: "flex", justifyContent: "center", alignItems: "flex-start", paddingTop: 32, paddingBottom: 32, marginLeft: 320, marginRight: 340 }}>
        <div style={{ position: "relative" }}>
          {/* Phone chrome */}
          <div style={{
            width: 410, background: "#1a1a2e", borderRadius: 44, padding: "12px 10px",
            boxShadow: "0 0 0 1px #2a2a3e, 0 20px 60px rgba(0,0,0,.25), 0 0 120px rgba(0,0,0,.06)"
          }}>
            {/* Notch */}
            <div style={{ width: 120, height: 6, background: "#2a2a3e", borderRadius: 3, margin: "0 auto 8px" }} />
            {/* Screen container */}
            <App onAdminToggle={() => setIsAdmin(true)} />
            {/* Home indicator */}
            <div style={{ width: 134, height: 5, background: "#3a3a4e", borderRadius: 3, margin: "8px auto 4px" }} />
          </div>
        </div>
      </main>
      ...
      <aside style={{ width: 340, flexShrink: 0, background: "#fff", borderLeft: "1px solid #e5e7eb", position: "fixed", top: 0, right: 0, bottom: 0, overflowY: "auto", padding: "24px 20px", scrollbarWidth: "none", msOverflowStyle: "none" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {c.salesCards.map((card, i) => {
            ...
            const IconComp = isAdminCard ? Shield : getIcon(card.icon);
            return (
              <div key={i} style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 14, padding: "18px 16px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                  ...
                  <h3 style={{ fontFamily: "'Vollkorn', serif", fontSize: 18, fontWeight: 600, color: "#1a1a1a", margin: 0 }}>{card.title}</h3>
                </div>
                <p style={{ fontSize: 12, color: "#6b7280", lineHeight: 1.6, margin: 0 }}>{card.description}</p>
                {isAdminCard && (
                  <button onClick={() => setIsAdmin(true)} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, width: "100%", padding: "10px 0", marginTop: 14, borderRadius: 8, border: "none", background: accent, color: "#fff", fontWeight: 700, fontSize: 14, cursor: "pointer" }}>
          ...
          {/* CTA Card */}
          <div style={{ background: `linear-gradient(135deg, ${accent}18, ${accent}08)`, border: `1px solid ${accent}30`, borderRadius: 14, padding: "22px 18px", textAlign: "center" }}>
            <h3 style={{ fontFamily: "'Vollkorn', serif", fontSize: 22, fontWeight: 600, color: "#1a1a1a", margin: "0 0 6px" }}>{c.cta.heading}</h3>
